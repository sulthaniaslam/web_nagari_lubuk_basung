    ...
    <div class="container">
      ...
      <div class="row justify-content-between align-items-center mb-4 g-3 text-center text-md-start">
        <div class="col-md-7 animate__animated animate__fadeInLeft">
          ...
          <h1 class="section-title-pro mb-2">Data Kegiatan</h1>
          <p class="section-subtitle-custom">Daftar keterbukaan data Kegiatan untuk publik.</p>
        </div>
        <div class="col-md-4 animate__animated animate__fadeInRight">
          <div class="input-group search-table-box">
            <span class="input-group-text bg-white border-end-0 text-muted"><i class="fa-solid fa-magnifying-glass"></i></span>
            {{-- Input Search multiguna (Judul / Tahun) --}}
            <input type="text" class="form-control border-start-0" id="searchDocument" placeholder="Cari nama dokumen atau tahun...">
          </div>
        </div>
      </div>
      ...
      <div class="glass-card bento-card border-0 p-0 overflow-hidden animate__animated animate__fadeInUp">
        <div class="table-responsive">
          <table class="table custom-executive-table mb-0" id="lpjTable">
            <thead>
              <tr>
            ...
            <tbody>
                @if (count($data['pkk']['kegiatan_pkk']) == 0)
                <tr id="noDataRow">
                ...
                @foreach ($data['pkk']['kegiatan_pkk'] as $item)
                <tr class="data-row">
                    <td class="text-center fw-bold text-muted row-number">{{ $loop->iteration }}</td>
                    <td class="searchable-cell">
                        <span class="fw-bold text-darkblue d-block document-title">{{ $item['judul_kegiatan'] }}</span>
                        <span class="badge bg-light text-secondary border mt-1">
                          {{-- Ditambahkan class 'document-year' agar bisa dibaca JavaScript --}}
                          <i class="fa-regular fa-calendar-check me-1"></i> Tahun Periode: <span class="document-year">{{ $item['created_at'] }}</span>
                        </span>
                    </td>
                    <td class="text-center">
                      <span class="badge badge-green"><i class="fa-solid fa-circle-check me-1"></i> Sudah Terbit</span>
                    </td>
                    <td class="text-center">
                        <div class="d-flex justify-content-center gap-2">
                          @if(!empty($item['gambar_kegiatan']))
                            <button type="button" 
                              class="btn btn-sm btn-action-view btn-preview-trigger" 
                              data-bs-toggle="modal" 
                              data-bs-target="#pdfPreviewModal" 
                              data-url="{{ env('API_STORAGE') . $item['gambar_kegiatan'] }}" 
                              data-title="{{ $item['judul_kegiatan'] }}"
                              title="Pratinjau Dokumen">
                              <i class="fa-solid fa-eye"></i>
                            </button>
                            
                            <a href="{{ env('API_STORAGE') . $item['gambar_kegiatan'] }}" 
                              download="{{ $item['judul_kegiatan'] }}" 
    ...
    if (searchInput) {
      searchInput.addEventListener('input', function () {
        ...
        filteredRows = Array.from(tableRows).filter(row => {
          ...
          return docTitle.includes(searchTerm) || docYear.includes(searchTerm);
        });
